...
{
    public partial class TimkiemchitraTBSX : Form
    {
        public TimkiemchitraTBSX()
        {
            InitializeComponent();
        }

        private void TimkiemchitraTBSX_Load(object sender, EventArgs e)
        {
            Class.FunctionHuong.Connect();
            ResetValues();
            DataGridView.DataSource = null;
            Class.FunctionHuong.FillCombo("select manhanvien,tennhanvien from tblNhanVien WHERE machucvu IN ('CV03')", cboNhanvien, "manhanvien", "tennhanvien");
            cboNhanvien.SelectedIndex = -1;
            Class.FunctionHuong.FillCombo("select mancc,tenncc from tblNhaCungCap", cboNcc, "mancc", "tenncc");
            cboNcc.SelectedIndex = -1;
            // txtTongtien.Enabled = false;
        }
        DataTable tblTKChitra;
        ...
        private void btnTimkiem_Click(object sender, EventArgs e)
        {
            ...
            sql = @"
        SELECT hd.mahdc, nv.tennhanvien, hd.ngaychi, ncc.tenncc, 
               ISNULL(SUM(ct.dongia * ct.soluong), 0) AS tongtien
        FROM tblHoadonchitraTBSX hd
        LEFT JOIN tblNhanVien nv ON hd.manhanvien = nv.manhanvien
        LEFT JOIN tblNhaCungCap ncc ON hd.mancc = ncc.mancc
        LEFT JOIN tblChitiethoadonchitraTBSX ct ON hd.mahdc = ct.mahdc
        WHERE 1=1";

            //   sql = "SELECT * from tblHoadonnhapNVL where 1=1";
            if (txtMahoadon.Text != "")
                sql = sql + " AND hd.mahdc like N'%" + txtMahoadon.Text + "%'";//%: Đại diện cho 0 hoặc nhiều ký tự bất kỳ
            if (txtThang.Text != "")
                sql = sql + " AND month(ngaychi) =" + txtThang.Text;
            if (txtNam.Text != "")
                sql = sql + " AND year(ngaychi) =" + txtNam.Text;
            if (cboNhanvien.Text != "")
            ...
            sql += " GROUP BY hd.mahdc, nv.tennhanvien, hd.ngaychi, ncc.tenncc";
            if (txtTongtien.Text != "")
                sql += " HAVING ISNULL(SUM(ct.dongia * ct.soluong), 0) LIKE N'%" + txtTongtien.Text + "%'";

            tblTKChitra = Class.FunctionHuong.GetDataToTable(sql);
            if (tblTKChitra.Rows.Count == 0)
            {
                MessageBox.Show("Không có bản ghi thỏa mãn điều kiện!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ResetValues();
            }
            else
                MessageBox.Show("Có " + tblTKChitra.Rows.Count + " bản ghi thỏa mãn điều kiện!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            DataGridView.DataSource = tblTKChitra;
            Load_DataGridView();
        }
        ...
        {           
            ...
            DataGridView.Columns[2].HeaderText = "Ngày chi";
